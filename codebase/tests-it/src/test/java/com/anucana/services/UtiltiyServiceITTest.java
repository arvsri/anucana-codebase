...
public class UtiltiyServiceITTest implements ITypeConstants{
	...
	public void getTypesByGroup(){
		Collection<Type> types = utilityService.getTypesByGroup(TYPE_GRP_STATUS_CD);
		assertEquals(2, types.size());
		assertTypeCode(types,TYPE_STATUS_CD_ACT);
		assertTypeCode(types,TYPE_STATUS_CD_INACT);
		
		types = utilityService.getTypesByGroup(TYPE_GRP_LOGIN_STATUS);
		...
		types = utilityService.getTypesByGroup(TYPE_GRP_USER_ROLES);
		...
		types = utilityService.getTypesByGroup(TYPE_GRP_PHONE_TYPE_CD);
		assertEquals(3, types.size());
		assertTypeCode(types,TYPE_PHONE_WORK);
		assertTypeCode(types,TYPE_PHONE_HOME);
		assertTypeCode(types,TYPE_PHONE_MOBILE);

		types = utilityService.getTypesByGroup(TYPE_GRP_MESSENGER_TYPE_CD);
		assertEquals(3, types.size());
		...
		types = utilityService.getTypesByGroup(TYPE_GRP_INDUSTRY_TYPE_CD);
		assertEquals(147, types.size());

		types = utilityService.getTypesByGroup(TYPE_GRP_COUNTY_CD);
